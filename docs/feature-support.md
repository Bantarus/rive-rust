...
1. **Bevy-side read-back in `zero_copy`** — under `zero_copy` the advance runs in the render world,
   so every read (view-model **watch**/**observe**, bone/solo/constraint getters, `focus_state`, the
   live playhead) is `floor`-only today. Build the first render→main back-channel; wire **view-model
   watch + observe** first (the documented modern event path, verifiable with the scripting asset),
   then the rig/focus reads. *(Type-specific constraint props — IK invert/parentBoneCount, distance
   distance/mode, follow-path distance/orient/offset — shipped.)*
