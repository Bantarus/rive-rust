...
rem ===========================================================================
rem  run-windows-voxelien.cmd - native Windows launcher for the Voxelien face
rem  live viewer (the `voxelien_face` floor-tier example in bevy-rive).
rem
...
rem    scripts/sync_to_windows.sh && cmd.exe /c "scripts\run-windows-voxelien.cmd"
rem
...
if "%~1"=="" (
    set "RIVE_RIV=voxelien_face_published.riv"
) else (
    set "RIVE_RIV=%~1"
)
...
if not exist "%VSWHERE%" (
    echo [run-windows-voxelien] ERROR: vswhere.exe not found at "%VSWHERE%".
...
if not defined VSINSTALL (
    echo [run-windows-voxelien] ERROR: no Visual Studio install with the x64 C++ toolset found.
...
if not exist "%VCVARS%" (
    echo [run-windows-voxelien] ERROR: vcvars64.bat not found at "%VCVARS%".
    exit /b 9009
)
call "%VCVARS%" >nul
if errorlevel 1 (
    echo [run-windows-voxelien] ERROR: vcvars64.bat failed ^(exit %errorlevel%^).
    exit /b %errorlevel%
)
...
if exist "%VSINSTALL%\VC\Tools\Llvm\x64\bin\clang-cl.exe" (
    ...
    echo [run-windows-voxelien] WARNING: clang-cl.exe not found under the VS install. Install the
...
pushd "%~dp0.." || (
    echo [run-windows-voxelien] ERROR: could not cd to repo root from "%~dp0".
    exit /b 1
)
echo [run-windows-voxelien] VS:    "%VSINSTALL%"
echo [run-windows-voxelien] repo:  "%CD%"
echo [run-windows-voxelien] RIVE_RIV=!RIVE_RIV!
echo [run-windows-voxelien] cargo run --release -p bevy-rive --example voxelien_face --features floor

cargo run --release -p bevy-rive --example voxelien_face --features floor
