...
- **zero-copy watch read-back — now SHIPS** (the render→main back-channel: the node reads
  ...
  of latency vs floor's same-frame reads). The channel is the template for the remaining
  `zero_copy` reads (rig getters, `focus_state`, playhead — backlog #1).
