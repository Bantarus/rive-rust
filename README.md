...
control, nested-artboard access, view-model list structural mutation + instance construction,
and the render→main read-back channel carrying view-model watch/observe, rig
(bone/constraint/solo), focus-state, live playhead/duration, and text-run reads in both tiers.
See the [feature matrix](docs/feature-support.md) for the roadmap.
