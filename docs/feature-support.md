...
*(All ROI-ordered backlog items are COMPLETE — see the per-feature rows above and the notes below.)*

*(Backlog item "Constructing anonymous inline nested VM types" is COMPLETE:
`Artboard::view_model_by_property_path(root, path)` descends `viewModel`-typed properties by
REFERENCE-ID (never by name) from a named root view model to reach a nested type's DEFINITION,
returning a `RiveViewModelRuntime` the existing `create_instance*` mints — so a nested type is
constructed by NAVIGATING to it (by the property it fills), not by needing its top-level name.
`NewViewModel::nested_blank`/`nested_default` expose it in the `RiveViewModel` component (both
tiers, riding the already-proven AddNew/ReplaceViewModel ferry). Render-proven on the slot machine:
constructing `WheelCenter`'s type by descent (`view_model_by_property_path("Chicken", "WheelCenter")`, and
the two-level `"SlotMachine"` / `"Chicken/WheelCenter"`) then replacing is BYTE-IDENTICAL
(`0b39fa1c`) to constructing it by name (`view_model_by_name("Wheel")`), and both differ from
baseline (`ee6986da`) — a real, functional instance. FINDING: no available asset (the 10 demos +
the vendored data-binding suite) authors a genuinely NAMELESS view-model type — every type in a
file's view-model table has a unique name, so the original "anonymous `IconType`" premise was a
misread of a property display-name (it resolves to the named `Icons`). The descent reaches a
nameless type identically (it resolves by id — the name is never consulted, proven by the
name-independent equivalence), so that sub-case is mechanism-proven; its real value is decoupling
construction from a hardcoded type name.)*
...
- **list STRUCTURAL mutation (add/remove/swap) + INSTANCE construction — now SHIP**: mint fresh
  ...
  `replace_view_model`) in BOTH tiers. Render-proven on the slot-machine `WheelList`. **Nested-type
  construction** also ships (`Artboard::view_model_by_property_path` / `NewViewModel::nested_*`): descend
  `viewModel`-typed properties by reference-id to a nested type's definition, reached without its
  top-level name.
